...
if (scalar(@ARGV) > 0) {
  ...
  my $FEATURE_TESTS = {
    't/modules/mod_case/copy.t' => {
      order => ++$order,
      test_class => [qw(mod_case mod_copy)],
    },

    't/modules/mod_case/sftp.t' => {
      order => ++$order,
      test_class => [qw(mod_case mod_sftp)],
    },

    't/modules/mod_case/tls.t' => {
      order => ++$order,
      test_class => [qw(mod_case mod_tls)],
    },
  };
