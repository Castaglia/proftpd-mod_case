...
sub caseignore_site_copy_issue5 {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
  my $setup = test_setup($tmpdir, 'case');

  my $test_dir = File::Spec->rel2abs("$setup->{home_dir}/TeSt.D/sUb.d");
  ...
  if ($pid) {
    eval {
      ...
      my ($resp_code, $resp_msg) = $client->site('COPY', 'test.d/sub.d/SrC.txt', 'test.d/sub.d/dst.txt');
